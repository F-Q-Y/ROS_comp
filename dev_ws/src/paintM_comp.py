...
for i in [1,3,5]:
    base_route = base
    for j in range(1,21):
        file_route = base_route+"/M_"+str(i)+"_N_"+str(j)+"_sum_writer.out"
        f = open(file_route)
        lines = f.readlines()
        sum = 0.0
        for line in lines:
...
plt.plot(x_new,comp_1_smo,color='springgreen',label='comp_proj:M=1')
plt.plot(x_new,comp_3_smo,color='royalblue',label='comp_proj:M=3')
plt.plot(x_new,comp_5_smo,color='yellow',label='comp_proj:M=5')
plt.legend()

plt.xlabel('N')
plt.ylabel('Time consuming for a single pub/s')
plt.savefig("./const_M_comp.png")
plt.show()